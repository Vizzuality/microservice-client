'use strict';
var debug = require('debug')('register-microservice-client');
...
var register = function(opts) {
    debug('Registering microservice');
    ...
    if (process.env.SELF_REGISTRY === 'on') {
        ...
        co(function*() {
            debug('Obtaining info to register microservice');
            var packageJSON = require('../../package.json');
            debug('package', packageJSON);
        ...
        }).then(function(){console.log('Salio');}, function(err){
            console.error(err);
            throw err;
        });
    }
