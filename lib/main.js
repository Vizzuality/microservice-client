...
var exitHandler = function(signal, opts) {
    logger.debug('Signal ', signal);
    co(function*() {
        // yield unregister(opts);
...
var register = function(opts) {
    return co(function*() {
        ...
        opts.app.use(function*(next){
            if(this.path === '/info'){
                logger.info('Obtaining info to register microservice with token %s', this.query.token);

                authenticationToken = this.query.token;

                logger.info('ip', this.ip);
                logger.info('todo', this);
        ...
        //
        //         let result = yield request({
        //             uri: process.env.API_GATEWAY_URI + '/gateway/service',
        //             method: 'POST',
        //             json: true,
        //             body: serviceConfig
        //         });
        ...
        //         }
        //
        //         authenticationToken = result.body.token;
        //         logger.info('Register service in API Gateway correct!');
        //         process.on('exit', exitHandler.bind(this, 'exit', opts));
        //         process.on('SIGINT', exitHandler.bind(this, 'SIGINT', opts));
        //         process.on('SIGTERM', exitHandler.bind(this, 'SIGTERM', opts));
        //         // process.on('SIGKILL', exitHandler.bind(this, 'SIGKILL'));
        //         process.on('uncaughtException', exitHandler.bind(this, 'uncaughtException', opts));
        //
        //     } catch (e) {
...
var requestToMicroservice = function *(config){
    ...
    config.uri = process.env.API_GATEWAY_URI + config.uri ;
