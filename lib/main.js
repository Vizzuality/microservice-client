    ...
    return co(function*() {
        ...
        opts.app.use(function*(next){
            if(this.path === '/info'){
                ...
                if(opts.callbackUpdate){
                    logger.debug('Calling to callback update');
                    opts.callbackUpdate(getInfo());
                }
                return;
            }
            yield next;
 ...
 * IMPORTANT!!! If send uri param then the library is NOT concat api-gateway url.
 * If you don't know api-gateway url, you set the microservice path in path attribute
...
var requestToMicroservice = function *(config){
    ...
    if(!config.uri){
        config.uri = apiGatewayUrl + config.path ;
    }
